    ...
    async function beautifyMuv(text) {
      ...
      try {
        if (/<StatusPlaceHolderImpl\s*\/>/i.test(normalized)) {
          ...
          if (sb) { ensureStatusCss(); return normalized.replace(/<StatusPlaceHolderImpl\s*\/>/gi, '<div class="muv-statusbar-wrap">' + sb + '</div>') }
        }
        // 没有卡片数据也要能出状态栏：级联不依赖卡片，只要能解析出结构
        const cascaded = await cascadeStatusBlock(normalized, null)
        if (cascaded !== normalized) return cascaded
      } catch (_) {}
      ...
      return replaceChoices(normalized)
    }
    ...
    async function cascadeStatusBlock(text, cardJson) {
      ...
      try {
        ...
        const frame = isCardHtml
          ? '<iframe id="' + id + '" class="muv-iframe" srcdoc="' + escAttr(d.html) + '" sandbox="allow-scripts" style="width:100%;height:600px;border:none;border-radius:8px;background:transparent"></iframe>'
          : d.html
