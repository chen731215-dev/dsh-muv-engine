...
'.muv-char-name{display:inline-block;font-weight:700;color:#ffdd99}.muv-audio,.muv-video-ph{display:flex;align-items:center;gap:6px;padding:6px 10px;background:rgba(122,184,255,.06);border-radius:6px;font-size:12px;color:var(--dsw-alias-label-secondary)}'+
      ...
      function muvFindChoiceRange(root) {
        ...
        var full = walked.text
        var nodes = walked.map
        var m = /<choices?>([\s\S]*?)<\/choices?>/i.exec(full)
        if (!m) return null
        // 取「起点不晚于 pos 的最后一个文本节点」：pos 若落在合成的换行符上，
        // 会吸附到前一个文本节点的末尾（删除/插入的位置因此仍然正确）。
        ...
        var a = locate(m.index)
        var b = locate(m.index + m[0].length)
      ...
      function muvSanitizeNode(md) {
        ...
        try { muvCleanText(md); muvRenderOpts(md); muvStyleTavernOpts(md); muvRenderChoices(md) } catch(e) { console.error('[muv-engine sanitize]', e) }
      }
