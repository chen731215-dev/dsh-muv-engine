...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var MUV_SB_CSS = '.muv-sb{margin:12px 0;border:1px solid var(--dsw-alias-border-l2,rgba(127,127,127,.25));border-radius:10px;overflow:hidden;font-size:13px}'
      + '.muv-sb-hd{display:flex;flex-wrap:wrap;gap:10px;padding:9px 13px;background:var(--dsw-alias-bg-l2,rgba(127,127,127,.08));font-weight:600}'
      + '.muv-sb-hd span{opacity:.85}'
      + '.muv-sb-bd{padding:10px 13px;display:flex;flex-wrap:wrap;gap:8px}'
      + '.muv-sb-chip{padding:2px 9px;border-radius:999px;background:var(--dsw-alias-bg-l2,rgba(127,127,127,.1));white-space:nowrap}'
      + '.muv-sb-row{display:flex;align-items:center;gap:8px;margin:5px 0}'
      + '.muv-sb-name{width:5.5em;flex:none;opacity:.85}'
      + '.muv-sb-bar{flex:1;height:7px;border-radius:999px;background:var(--dsw-alias-bg-l2,rgba(127,127,127,.15));overflow:hidden}'
      + '.muv-sb-fill{height:100%;border-radius:999px;background:linear-gradient(90deg,#8b5cf6,#ec4899)}'
      + '.muv-sb-val{width:2.6em;flex:none;text-align:right;opacity:.7;font-variant-numeric:tabular-nums}'
    ...
    function buildDefaultStatusBar(text) {
      ...
      for (var j = 0; j < affs.length; j++) {
        ...
        bars += '<div class="muv-sb-row"><span class="muv-sb-name">' + escHtml(n) + '</span>'
          + '<span class="muv-sb-bar"><span class="muv-sb-fill" style="width:' + v + '%"></span></span>'
      ...
      if (bars) html += '<div class="muv-sb-bd" style="display:block">' + bars + '</div>'
      return html + '</div>'
    }
    ...
    async function beautifyMuv(text) {
      ...
      try {
        ...
        if (cardJson) {
          ...
          if (d.ok) {
            let result = d.text
            // ★ 状态栏：卡片自带「状态栏」正则脚本时用它的 HTML（iframe 沙箱渲染）；
            //   没有就用内置模板，直接从本轮 _.set/_.add 变量生成状态卡（内联，继承 DSH 主题变量）
            let sbHtml = d.statusBarHtml
            if (!sbHtml && /<StatusPlaceHolderImpl\s*\/>/i.test(result)) sbHtml = buildDefaultStatusBar(text)
            if (sbHtml) {
              ensureStatusCss()
              ...
              const frame = builtin
                ? sbHtml
                : '<iframe id="' + id + '" class="muv-iframe" srcdoc="' + escAttr(sbHtml) + '" sandbox="allow-scripts" style="width:100%;height:600px;border:none;border-radius:8px;background:transparent"></iframe>'
      ...
      try {
        if (/<StatusPlaceHolderImpl\s*\/>/i.test(text)) {
          const sb = buildDefaultStatusBar(text)
          if (sb) { ensureStatusCss(); return text.replace(/<StatusPlaceHolderImpl\s*\/>/gi, '<div class="muv-statusbar-wrap">' + sb + '</div>') }
        }
      } catch (_) {}
