...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     *    卡里的东西，收到多少都只是"多高"。
     *
     * 测量用 `documentElement.scrollHeight` 与 `body.scrollHeight` 的最大值；`load`
     * 后测一次，再用 ResizeObserver（150ms 去抖）跟随后续变化——卡是 JS 驱动的，
     * 内容会自己长高。最后两个 setTimeout 是给"没有 ResizeObserver 或它没触发"兜底。
     * @returns {string}
     */
      ...
      try {
        ...
        if (cardJson) {
          ...
          if (d.ok) {
            ...
            // 占位符还在才构造 iframe：卡自己的正则往往已经把占位符换掉了，那时
            // 下一页 210 KB 的 escAttr 会被下一行的 replace 直接丢掉——纯浪费。
            if (sbHtml && /<StatusPlaceHolderImpl\s*\/>/i.test(result)) {
              ensureStatusCss()
              const builtin = /^\s*<div class="muv-sb"/.test(sbHtml)
              const frame = builtin ? sbHtml : cardHtmlIframe(sbHtml)
