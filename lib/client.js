  ...
  factory: (require) => {
    ...
    function renderFencedHtml(text) {
      ...
      while ((m = open.exec(source))) {
        ...
        if (isDoc) {
          out += '<div class="muv-statusbar-wrap"><iframe class="muv-iframe" srcdoc="' +
    ...
    function renderMediaTags(text) {
      ...
      var sm
      while ((sm = scriptRe.exec(source))) scriptRanges.push([sm.index, scriptRe.lastIndex])
      var inScript = function (i) {
        for (var k = 0; k < scriptRanges.length; k++) {
          if (i >= scriptRanges[k][0] && i < scriptRanges[k][1]) return true
        }
        return false
      }
      var out = ''
