...
export function parseYamlStatusBlock(body) {
  ...
  if (chars.length) {
    ...
    for (const c of chars) {
      // 角色名要有自己的块：以前它只是两块字段之间飘着的一行加粗字（无卡片、无边框），
      // 而 CSS 里的 `.muv-sb-char` / `.muv-sb-char-name` 从来没被这段渲染器用到过
      // （只有 loose 级联在用）—— "定义了样式却没人输出" 这类错配单测看不见。
      // `.muv-sb-name` 一并保留：它是 `.muv-sb-row` 里那套「名字 + 窄条/状态」布局
      // 的一部分，旧断言也按它计数。
      parts.push('<div class="muv-sb-char">')
      parts.push('<div class="muv-sb-row"><span class="muv-sb-name muv-sb-char-name">👤 ' + esc(stripLeadingEmoji(c.name)) + '</span></div>')
...
export function parseFreeStatusBlock(body) {
  ...
  if (chars.length) {
    parts.push('<div class="muv-sb-sub">')
    for (const c of chars) {
      // 同 yaml 级：角色名包进 `.muv-sb-char` 块，并带上 `.muv-sb-char-name`
      // （否则 CSS 里那三个类在 yaml/free 这两个最常走到的级联里永远是死的）。
      parts.push('<div class="muv-sb-char">')
      parts.push('<div class="muv-sb-row"><span class="muv-sb-name muv-sb-char-name">👤 ' + esc(c.name) + '</span>'
